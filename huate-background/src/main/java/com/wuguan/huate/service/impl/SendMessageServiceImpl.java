	...
	public void propertyPayFeeMessageSend() {
		...
		if (houses.size()!=0) {
			for (HouseM houseM : houses) {
				...
				//thing2.put("value", houseM.getBuilding()+"栋"+houseM.getUnit()+"单元"+houseM.getRoom());
				Map<String,Object> thing3 = new HashMap<String, Object>();//缴费类型 物业费
				thing3.put("value", houseM.getBuilding()+"栋"+houseM.getUnit()+"单元"+houseM.getRoom()+"物业管理服务费");
				...
				scheduledThreadPool.execute(new Runnable() {
					@Override
					public void run() {
						baseService.subscribeMessageSend(accessToken, houseM.getOpenid(),"pages/order/order/order?houseNo="+houseM.getHouseNo()+"&feeType=1",Constant.PROPERTY_PAY_TEMPLATEID, params);
	...
	public void ParkPayFeeMessageSend() {
		...
		if (parks.size()!=0) {
			for (ParkM parkM : parks) {
				...
				//thing2.put("value", parkM.getCarNo()!=null?parkM.getCarNo():parkM.getParkNo());
				Map<String,Object> thing3 = new HashMap<String, Object>();//缴费类型 物业费
				...
				scheduledThreadPool.execute(new Runnable() {
					@Override
					public void run() {
						baseService.subscribeMessageSend(accessToken, parkM.getOpenid(),"pages/park/order/order?parkNo="+parkM.getParkNo(),Constant.PROPERTY_PAY_TEMPLATEID, params);
						Park park = new Park();
		...
		if (user.size()!=0) {
			for (UserVo userVo : user) {
				...
				scheduledThreadPool.execute(new Runnable() {
					@Override
					public void run() {
						baseService.subscribeMessageSend(accessToken, userVo.getOpenid(),"pages/home/home",Constant.NOTICE_TEMPLATEID, params);
					}
				});
			}
		}
