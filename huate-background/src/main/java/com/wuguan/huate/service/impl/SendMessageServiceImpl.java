...
public class SendMessageServiceImpl implements SendMessageService {
	 ...
	 */
	@Override
	public void consumesPayFeeMessageSend(List<HouseConsume> sendMessages) {
		if (sendMessages.size() != 0) {
			List<String> list = new ArrayList<String>();
			for (HouseConsume houseConsume : sendMessages) {
				if (!list.contains(houseConsume.getHouseNo())) {
					list.add(houseConsume.getHouseNo());
				}
			}
			List<UserHouseVo> userHouses = houseService.queryBindUser(list);
			for (UserHouseVo userHouseVo : userHouses) {
				for (HouseConsume consume : sendMessages) {
					if (userHouseVo.getHouseNo().equals(consume.getHouseNo())) {
						Boolean bool = redisHelper.existsKey("accessToken");
						if (!bool) {
							accessToken = baseService.authGetAccessToken(Constant.XJ_APPID, Constant.XJ_SECRET);
							redisHelper.set("accessToken", accessToken, 60 * 60 * 2);
						...
						String name = userHouseVo.getType() == HouseEnums.TypeEnum.BUSINESS.getValue()
								? userHouseVo.getOwner()
								: userHouseVo.getBuilding() + "栋" + userHouseVo.getUnit() + "单元"
										+ userHouseVo.getRoom();
						String feeName = consume.getType() == FeeNormEnums.FeeTypeEnum.WATER.getValue() ? "水费" : "电费";
						thing3.put("value", name + feeName);
						Map<String, Object> thing7 = new HashMap<String, Object>();// 公司名称
						thing7.put("value", "四川华特物业服务有限公司");
						Map<String, Object> thing5 = new HashMap<String, Object>();// 温馨提示
						thing5.put("value", "您好，本月的水电费已上传，请及时缴纳费用");
						Map<String, Object> params = new HashMap<String, Object>();
						// params.put("thing2", thing2);
						params.put("thing3", thing3);
						params.put("thing7", thing7);
						...
						scheduledThreadPool.execute(new Runnable() {
							@Override
							public void run() {
								baseService.subscribeMessageSend(accessToken, userHouseVo.getOpenid(),
										"pages/order/order/order?houseNo=" + consume.getHouseNo() + "&feeType="
												+ consume.getType(),
										Constant.PROPERTY_PAY_TEMPLATEID, params, Constant.XJ_APPID,
										Constant.XJ_SECRET);
							}
						});
