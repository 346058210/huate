...
public class FeeCountServiceImpl<V> implements FeeCountService {
	...
	public Map<String, Object> propertyFeeCount(String houseNo) {
		...
		boolean bool = (DateUtils.getDaysBetween(house.getDueTime(), new Date()) > 0) ? true : false;// 物业
		boolean rubbish = (DateUtils.getDaysBetween(house.getRubbishDueTime(), new Date()) > 0) ? true : false;// 垃圾
		...
		} else if (rubbish) {// 欠垃圾费
			PropertyOrderDetail liveDetail = new PropertyOrderDetail();
			for (FeeNorm norm : norms) {
				if (house.getRubbishTypeId() == norm.getId()) {
					
					Double money = (norm.getMtc() == FeeNormEnums.MtcEnum.INHERENT.getValue())
							? (norm.getPrice() * DateUtils.getMonthDiff(house.getRubbishDueTime(), new Date()))
							: (norm.getPrice() * DateUtils.getMonthDiff(house.getRubbishDueTime(), new Date())
									* house.getArea());
					liveDetail.setHouseId(house.getId());
					liveDetail.setName("应缴生活垃圾费");
					liveDetail.setTimeFrame(DateUtils.getYearMonth(house.getRubbishDueTime()) + "-"
		...
		map.put("rubbishFeeType", FeeNormEnums.FeeTypeEnum.RUBBISH.getValue());
		map.put("rubbishDueTime", house.getRubbishDueTime());// 到期时间
		map.put("state", house.getState());// 1 自住 2 出租 3 空置
		return map;
	}
	...
	public List<ConsumeOrderDetail> waterElectricFeeCount(String houseNo, Integer type) {
		...
		if (list.size() != 0) {
			...
			for (HouseConsume consume : list) {
				...
				detail.setStartQuan(consume.getStartQuan());
				detail.setEndQuan(consume.getEndQuan());
				payable.add(detail);
			}
			return payable;
		}
		...
		if (houses.size() != 0) {
			for (House house : houses) {
				...
				if (bool) {// 欠物业|生活垃圾费
					...
					Map<String, Object> map = new HashMap<String, Object>();
					map.put("name", "应缴生活垃圾费");
					map.put("money", new Double(money * 100).intValue());
					map.put("dueTime", house.getRubbishDueTime());
					map.put("houseId", house.getId());
					map.put("owner", house.getOwner());
					map.put("houseNo", house.getHouseNo());
					map.put("type", house.getType());// 到期时间
					map.put("timeFrame", DateUtils.getYearMonth(house.getRubbishDueTime()) + "-"
							+ DateUtils.getYearMonth(new Date()));
					map.put("feeType", FeeNormEnums.FeeTypeEnum.RUBBISH.getValue());
					array.add(map);
				}
				// 水
