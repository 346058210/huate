...
*/
@Service
public class FeeCountServiceImpl<V> implements FeeCountService {
	@Autowired
		...
		if (bool) {//存在欠费
			...
			for (FeeNorm norm : norms) {
				if (house.getPropertyTypeId()==norm.getId()) {
					rule.add(norm);
					Double money=(norm.getMtc()==FeeNormEnums.MtcEnum.INHERENT.getValue())?(norm.getPrice()*DateUtils.compareDate(house.getDueTime(), new Date(), 1)):(norm.getPrice()*DateUtils.compareDate(house.getDueTime(), new Date(), 1)*house.getArea());
					propertyDetail.setHouseId(house.getId());
					propertyDetail.setName("应缴物业费");
					propertyDetail.setTimeFrame(DateUtils.getYearMonth(house.getDueTime())+"-"+DateUtils.getYearMonth(new Date()));
					...
					liveDetail.setTimeFrame(DateUtils.getYearMonth(house.getDueTime())+"-"+DateUtils.getYearMonth(new Date()));
	...
	* @return
	...
	public Map<String, Object> parkFeeCount(String parkNo) {
		...
		if (bool) {//欠费
			...
			for (FeeNorm norm : norms) {
				if (park.getNormId()==norm.getId()) {
					rule.add(norm);
					Double money=(norm.getMtc()==FeeNormEnums.MtcEnum.INHERENT.getValue())?(norm.getPrice()*DateUtils.compareDate(park.getDueTime(), new Date(), 1)):(norm.getPrice()*DateUtils.compareDate(park.getDueTime(), new Date(), 1)*park.getArea());
					...
					detail.setTimeFrame(DateUtils.getYearMonth(park.getDueTime())+"-"+DateUtils.getYearMonth(new Date()));
		...
		Integer feeType=(park.getType()==ParkEnums.TypeEnum.RENT.getValue())?FeeNormEnums.FeeTypeEnum.PARKRENT.getValue():FeeNormEnums.FeeTypeEnum.PARKMANAGE.getValue();
		...
		if (houses.size()!=0) {
			for (House house : houses) {
				...
				if (bool) {//欠物业|生活垃圾费
					...
					for (FeeNorm norm : norms) {
						if (house.getPropertyTypeId()==norm.getId()) {
							money+=(norm.getMtc()==FeeNormEnums.MtcEnum.INHERENT.getValue())?(norm.getPrice()*DateUtils.compareDate(house.getDueTime(), new Date(), 1)):(norm.getPrice()*DateUtils.compareDate(house.getDueTime(), new Date(), 1)*house.getArea());
						}else if (house.getRubbishTypeId()==norm.getId()) {
							money+=(norm.getMtc()==FeeNormEnums.MtcEnum.INHERENT.getValue())?(norm.getPrice()*DateUtils.compareDate(house.getDueTime(), new Date(), 1)):(norm.getPrice()*DateUtils.compareDate(house.getDueTime(), new Date(), 1)*house.getArea());
						}	 
					}
					Map<String,Object> map = new HashMap<String, Object>();
					//String prefix= house.getType()==HouseEnums.TypeEnum.BUSINESS.getValue()?house.getOwner():house.getHouseNo();
					map.put("name", "应缴物业费");
					...
					map.put("type", house.getType());//到期时间
					map.put("timeFrame", DateUtils.getYearMonth(house.getDueTime())+"-"+DateUtils.getYearMonth(new Date()));
					map.put("feeType", FeeNormEnums.FeeTypeEnum.PROPERTY.getValue());
					array.add(map);
				}
				//水
				List<HouseConsume> list = houseConsumeService.getHouseConsumeByHouseNo(house.getHouseNo(), FeeNormEnums.FeeTypeEnum.WATER.getValue());
				if (list.size()!=0) {
					Double money=0.0;
					for (HouseConsume consume : list) {
				...
				//电费
				List<HouseConsume> electrics = houseConsumeService.getHouseConsumeByHouseNo(house.getHouseNo(), FeeNormEnums.FeeTypeEnum.ELECTRIC.getValue());
				if (electrics.size()!=0) {
					Double money=0.0;
		...
		if (parks.size()!=0) {
			for (Park park : parks) {
				...
				if (bool) {
					...
					for (FeeNorm norm : norms) {
						if (park.getNormId()==norm.getId()) {
							money+=(norm.getMtc()==FeeNormEnums.MtcEnum.INHERENT.getValue())?(norm.getPrice()*DateUtils.compareDate(park.getDueTime(), new Date(), 1)):(norm.getPrice()*DateUtils.compareDate(park.getDueTime(), new Date(), 1)*park.getArea());
						}		
					}
					Map<String,Object> map = new HashMap<String, Object>();
					...
					Integer feeType=(park.getType()==ParkEnums.TypeEnum.RENT.getValue())?FeeNormEnums.FeeTypeEnum.PARKRENT.getValue():FeeNormEnums.FeeTypeEnum.PARKMANAGE.getValue();
					map.put("feeType", feeType);
					map.put("timeFrame", DateUtils.getYearMonth(park.getDueTime())+"-"+DateUtils.getYearMonth(new Date()));
					map.put("money",  new Double(money*100).intValue());
